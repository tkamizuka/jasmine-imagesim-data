...
.. code-block:: bash
  ...
  usage:
    mkimage.py [-h|--help] [--pd paramdir] --starplate star_plate.csv [--var variability.json] --det det.json --tel tel.json --ace ace.json [--acex acex.fits] [--acey acey.fits] --ctl ctl.json [--dft drift.json] --format format [--od outdir] [--overwrite]
  ...
  options:
   ...
   --acex acex.fits            fits file containing acex data.
   --acey acey.fits            fits file containing acey data.
...
if __name__ == '__main__':
    ...
    if control_params.effect.ace == 'real_no_lag':
        print("Loading ace data")
        acex_fits = pf.open(args['--acex'])[0]
        acey_fits = pf.open(args['--acey'])[0]
        acex = acex_fits.data
        acey = acey_fits.data
        dtace2 = acex_fits.header['ACE-TOTT']/acex_fits.header['NAXIS1'] 
    ...
    if control_params.effect.ace == 'real_no_lag':
        print("Making ace psf...")
        apsf = []
        tplate = control_params.tplate + detector.readparams.t_scan # exp. + (overhead + scan)
        for iplate in range(0, control_params.nplate):
            tstart = tplate*iplate + detector.readparams.t_overhead
            tend   = tstart + control_params.tplate # cp.tplate is the exposure time.

            istart = int(tstart/dtace2)
            iend   = int(tend/dtace2)

            theta  = np.copy(theta_full[:, istart:iend])
            h, x, y = np.histogram2d(
                np.round(theta[1]*detpix_scale/fp_scale),
                np.round(theta[0]*detpix_scale/fp_scale),
                bins=np.arange(-101, 101)+0.5, normed=True) 

            apsf.append(convolve2d(psf, h, mode='same')) 
        apsf = np.array(apsf)
        theta_full = theta_full * 0
    ...
    for i_star, line in enumerate(table_starplate):
        ...
        for iplate in tqdm.tqdm(range(0, control_params.nplate)):
            ...
            pixar_org = run_simpix(control_params, theta, interpix_local, flat_intrapix, psfin, psfcenter, psfscale, Nts_per_plate)
            mags = np.arange(12.0, 15.0, 0.5)
            for i_mag in range(0,6):
                pixar = scaling_pixar(pixar_org, mags[i_mag])
