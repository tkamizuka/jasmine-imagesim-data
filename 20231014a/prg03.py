...
for i in range(np.size(mags)):
    ...
    for j in range(np.size(stks)): 
        ...
        for bit in range(10,17):
            ...
            data -= mean_val
            if i == 0 and j == 0:
                ax.step(np.arange(-10,10)+0.5, data[928,1018:1038]/np.max(data[928,1018:1038]),label=f'{bit:2d}bit')
            else:
                ax.step(np.arange(-10,10)+0.5, data[928,1018:1038]/np.max(data[928,1018:1038]))
        ax.set_yscale('log')
